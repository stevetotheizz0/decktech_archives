...
First turn: Pull Carida, activate and get D*: War Room (if it´s not there get D*: DCR 327). If you have Brangus in hand deploy him there. Same goes if Dr.E is there (and no Brangus). Get that Endor:DB and CC :DB, but only against space decks. Against mains, check what DB´s they play. (You can use those dockings to retrieve w Brangus).If Emperor is in hand, deploy him to DB. Second Turn: if the first turn didn´t go as it´s supposed to do, try doing this now then. 
...
Crush - beat WYS, mains and stuff via pulling Monnok, cancel clash and reduce their battle destinies at CC:DB.
